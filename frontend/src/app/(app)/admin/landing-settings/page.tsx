          ...
          <div className="card settings-panel">
            ...
            {activeTab === "Hero Section" ? (
              <FormGrid>
                ...
                <Text label="Tiêu đề" value={settings.hero.title} onChange={(title) => patch("hero", { ...settings.hero, title })} />
                <Area label="Mô tả" value={settings.hero.description} onChange={(description) => patch("hero", { ...settings.hero, description })} />
            ...
            {activeTab === "About & Footer" ? <FormGrid>
              ...
              <Text label="Tiêu đề menu chân trang" value={settings.footer.menuTitle} onChange={(menuTitle) => patch("footer", { ...settings.footer, menuTitle })} />
              <Text label="Tiêu đề chính sách chân trang" value={settings.footer.policyTitle} onChange={(policyTitle) => patch("footer", { ...settings.footer, policyTitle })} />
              <Text label="Copyright" value={settings.footer.copyright} onChange={(copyright) => patch("footer", { ...settings.footer, copyright })} />
            </FormGrid> : null}

            {activeTab === "Lợi ích nổi bật" ? <Repeater title="Lợi ích" name="uspItems" items={settings.uspItems} onAdd={addItem} onRemove={removeItem}>{(item, index) => <FormGrid><Text label="Biểu tượng" value={item.icon || ""} onChange={(icon) => updateList("uspItems", index, { ...item, icon })} /><Text label="Tiêu đề" value={item.title} onChange={(title) => updateList("uspItems", index, { ...item, title })} /><Area label="Mô tả" value={item.description} onChange={(description) => updateList("uspItems", index, { ...item, description })} /><NumberInput label="Thứ tự hiển thị" value={item.displayOrder} onChange={(displayOrder) => updateList("uspItems", index, { ...item, displayOrder })} /><Check label="Đang hiển thị" checked={item.isActive} onChange={(isActive) => updateList("uspItems", index, { ...item, isActive })} /></FormGrid>}</Repeater> : null}
            {activeTab === "Dịch vụ & Menu" ? (
              <Repeater title="Dịch vụ" name="services" items={settings.services} onAdd={addItem} onRemove={removeItem}>
                {(item, index) => (
                  <FormGrid>
                    <Text label="Tiêu đề" value={item.title} onChange={(title) => updateList("services", index, { ...item, title })} />
                    <Area label="Mô tả" value={item.description} onChange={(description) => updateList("services", index, { ...item, description })} />
                    <MediaPicker label="Ảnh dịch vụ" folder="services" value={item.imageUrl || ""} required={item.isActive} altText={item.title} onChange={(imageUrl) => updateList("services", index, { ...item, imageUrl })} />
                    ...
                    <NumberInput label="Thứ tự hiển thị" value={item.displayOrder} onChange={(displayOrder) => updateList("services", index, { ...item, displayOrder })} />
                    <Check label="Đang hiển thị" checked={item.isActive} onChange={(isActive) => updateList("services", index, { ...item, isActive })} />
                  </FormGrid>
                )}
              </Repeater>
            ) : null}
            {activeTab === "Bảng giá nổi bật" ? <Repeater title="Bảng giá" name="pricingHighlights" items={settings.pricingHighlights} onAdd={addItem} onRemove={removeItem}>{(item, index) => <FormGrid><Text label="Tiêu đề" value={item.title} onChange={(title) => updateList("pricingHighlights", index, { ...item, title })} /><Area label="Mô tả" value={item.description} onChange={(description) => updateList("pricingHighlights", index, { ...item, description })} /><Text label="Giá hiển thị" value={item.priceText} onChange={(priceText) => updateList("pricingHighlights", index, { ...item, priceText })} /><Text label="Loại bàn" value={item.tableType || ""} onChange={(tableType) => updateList("pricingHighlights", index, { ...item, tableType })} /><Text label="Khung giờ" value={item.timeRange || ""} onChange={(timeRange) => updateList("pricingHighlights", index, { ...item, timeRange })} /><Text label="Nhãn nổi bật" value={item.badge || ""} onChange={(badge) => updateList("pricingHighlights", index, { ...item, badge })} /><NumberInput label="Thứ tự hiển thị" value={item.displayOrder} onChange={(displayOrder) => updateList("pricingHighlights", index, { ...item, displayOrder })} /><Check label="Đang hiển thị" checked={item.isActive} onChange={(isActive) => updateList("pricingHighlights", index, { ...item, isActive })} /></FormGrid>}</Repeater> : null}
            {activeTab === "Gallery" ? <Repeater title="Thư viện ảnh" name="gallery" items={settings.gallery} onAdd={addItem} onRemove={removeItem}>{(item, index) => <FormGrid><Text label="Tiêu đề" value={item.title} onChange={(title) => updateList("gallery", index, { ...item, title })} /><MediaPicker label="Ảnh thư viện" folder="gallery" value={item.imageUrl} required={item.isActive} altText={item.altText} onChange={(imageUrl) => updateList("gallery", index, { ...item, imageUrl })} /><Text label="Mô tả thay thế" value={item.altText} onChange={(altText) => updateList("gallery", index, { ...item, altText })} /><Text label="Chú thích ảnh" value={item.caption || ""} onChange={(caption) => updateList("gallery", index, { ...item, caption })} /><Text label="Loại ảnh" value={item.imageType} onChange={(imageType) => updateList("gallery", index, { ...item, imageType })} /><NumberInput label="Thứ tự hiển thị" value={item.displayOrder} onChange={(displayOrder) => updateList("gallery", index, { ...item, displayOrder })} /><Check label="Đang hiển thị" checked={item.isActive} onChange={(isActive) => updateList("gallery", index, { ...item, isActive })} /></FormGrid>}</Repeater> : null}
            {activeTab === "Review khách hàng" ? <><div className="state-card">Các mục dưới đây là review marketing tĩnh cũ trong Landing Settings, chỉ dùng fallback khi chưa có Customer Reviews được duyệt. Review thật được quản lý ở <a href="/admin/customer-reviews">Quản lý đánh giá</a>.</div><Repeater title="Review nổi bật trên trang chủ (fallback cũ)" name="reviews" items={settings.reviews} onAdd={addItem} onRemove={removeItem}>{(item, index) => <FormGrid><Text label="Tên khách" value={item.customerName} onChange={(customerName) => updateList("reviews", index, { ...item, customerName })} /><MediaPicker label="Ảnh đại diện" folder="reviews" value={item.avatarUrl || ""} altText={item.customerName} onChange={(avatarUrl) => updateList("reviews", index, { ...item, avatarUrl })} /><MediaPicker label="Ảnh check-in" folder="reviews" value={item.checkInImageUrl || ""} altText={item.customerName} onChange={(checkInImageUrl) => updateList("reviews", index, { ...item, checkInImageUrl })} /><NumberInput label="Số sao" value={item.rating} onChange={(rating) => updateList("reviews", index, { ...item, rating })} /><Area label="Nội dung" value={item.content} onChange={(content) => updateList("reviews", index, { ...item, content })} /><NumberInput label="Thứ tự hiển thị" value={item.displayOrder} onChange={(displayOrder) => updateList("reviews", index, { ...item, displayOrder })} /><Check label="Đánh giá nổi bật" checked={item.isFeatured} onChange={(isFeatured) => updateList("reviews", index, { ...item, isFeatured })} /><Check label="Đang hiển thị" checked={item.isActive} onChange={(isActive) => updateList("reviews", index, { ...item, isActive })} /></FormGrid>}</Repeater></> : null}
