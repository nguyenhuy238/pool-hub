...
export default function ProductsPage() {
  ...
  return (
    <>
      <PageHeader title="Products" description="Quản lý sản phẩm, SKU, giá và tồn kho." />
      <ListControls search={params.search} pageNumber={params.pageNumber} pageSize={params.pageSize} onChange={setParams} />
      <SmartForm<Product> title="Tạo sản phẩm" initial={{ stockQuantity: 0 }} fields={[{ name: "name", label: "Tên", required: true }, { name: "sku", label: "SKU", required: true }, { name: "productCategoryId", label: "Category ID", type: "number", required: true }, { name: "unitPrice", label: "Giá", type: "number", required: true }, { name: "stockQuantity", label: "Tồn kho", type: "number", required: true }]} onSubmit={async (value) => { await productApi.create(value); reload(); }} />
