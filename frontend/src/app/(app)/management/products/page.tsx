...
export default function ProductsPage() {
  ...
  return (
    <>
      ...
      <DataTable rows={rows as unknown as Record<string, unknown>[]} columns={[{ key: "name", label: "Tên" }, { key: "sku", label: "SKU" }, { key: "unitPrice", label: "Giá", render: (row) => money(Number(row.unitPrice)) }, { key: "stockQuantity", label: "Kho", render: (row) => Number(row.stockQuantity) <= 5 ? <Badge tone="red">{String(row.stockQuantity)} sắp hết</Badge> : String(row.stockQuantity) }]} actions={(row) => <button className="danger-btn" onClick={() => productApi.delete(Number(row.productId)).then(() => reload())}>Xóa</button>} />
    </>
  );
}
