...
export function BookingForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast("Yêu cầu đặt bàn đã được gửi. Nhân viên sẽ xác nhận trong thời gian sớm nhất.", "success");
      setForm(initialForm);
    } catch (err) {
  ...
  return (
    <section className="landing-section booking-section" id="booking">
      <div className="booking-copy">
        ...
        <p>Booking public được gửi về hệ thống và mặc định chờ xác nhận trước khi khách tới chơi.</p>
      </div>
      <form className="booking-form" onSubmit={submit}>
        ...
        <label><span>Ngày đặt *</span><input type="date" min={new Date().toISOString().slice(0, 10)} value={form.bookingDate} onChange={(e) => setForm({ ...form, bookingDate: e.target.value })} /></label>
        <label><span>Giờ bắt đầu *</span><input type="time" value={form.startTime} onChange={(e) => setForm({ ...form, startTime: e.target.value })} /></label>
        <label><span>Thời lượng</span><select value={form.durationHours} onChange={(e) => setForm({ ...form, durationHours: Number(e.target.value) })}><option value={1}>1 giờ</option><option value={2}>2 giờ</option><option value={3}>3 giờ</option><option value={4}>4 giờ</option></select></label>
        ...
        <button className="primary-btn full-field" disabled={saving}>{saving ? "Đang gửi..." : "Gửi yêu cầu đặt bàn"}</button>
      </form>
    </section>
  );
}
